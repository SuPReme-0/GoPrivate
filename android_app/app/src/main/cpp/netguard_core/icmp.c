...
void check_icmp_socket(const struct arguments *args, const struct epoll_event *ev) {
    ...
    if (ev->events & EPOLLERR) {
        ...
        if (ev->events & EPOLLIN) {
            ...
            // 🚨 HEAP FRAGMENTATION FIX: Stack-allocated buffer (Zero Malloc)
            ...
            if (bytes < 0) {
                if (errno != EINTR && errno != EAGAIN) s->icmp.stop = 1;
            } else if (bytes == 0) {
                s->icmp.stop = 1;
            } else {
...
jboolean handle_icmp(const struct arguments *args, const uint8_t *pkt, size_t length, const uint8_t *payload, int uid, const int epoll_fd) {
    ...
    if (cur == NULL) {
        struct ng_session *s = malloc(sizeof(struct ng_session)); // Using standard malloc now
        if (!s) return 0;
...
int open_icmp_socket(const struct arguments *args, const struct icmp_session *cur) {
    ...
    // 🚨 FD LEAK FIX: Ensure socket closes if Kotlin protection fails
...
ssize_t write_icmp(const struct arguments *args, const struct icmp_session *cur, uint8_t *data, size_t datalen) {
    ...
    if (cur->version == 4) {
        ...
        if (datalen) memcpy(buffer + sizeof(struct iphdr), data, datalen);
        ...
        if (datalen) memcpy(buffer + sizeof(struct ip6_hdr), data, datalen);
