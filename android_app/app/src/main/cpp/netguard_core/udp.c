...
int check_udp_session(const struct arguments *args, struct ng_session *s,
    ...
    if (s->udp.state == UDP_CLOSED && (s->udp.sent || s->udp.received)) {
        // 🚨 GHOST-STRING FIX: account_usage is a dead stub in our Kernel.
        // We pass empty strings instead of forcing the CPU to run inet_ntop.
        account_usage(args, s->udp.version, IPPROTO_UDP,
                      "", ntohs(s->udp.dest), s->udp.uid, s->udp.sent, s->udp.received);
        s->udp.sent = 0;
        s->udp.received = 0;
    }
...
void check_udp_socket(const struct arguments *args, const struct epoll_event *ev) {
    ...
    if (ev->events & EPOLLERR) {
        ...
        if (ev->events & EPOLLIN) {
            ...
            // 🚨 HEAP FRAGMENTATION FIX: Stack-allocated buffer
            uint8_t buffer[65536];
            ssize_t bytes = recv(s->socket, buffer, sizeof(buffer), 0);

            if (bytes < 0) {
                if (errno != EINTR && errno != EAGAIN) s->udp.state = UDP_FINISHING;
            } else if (bytes == 0) {
                s->udp.state = UDP_FINISHING;
            } else {
                ...
                if (write_udp(args, &s->udp, buffer, (size_t) bytes) < 0) {
                    ...
                    if (ntohs(s->udp.dest) == 53) s->udp.state = UDP_FINISHING;
                }
            }
        }
    }
...
int has_udp_session(const struct arguments *args, const uint8_t *pkt, const uint8_t *payload) {
    ...
    if (ntohs(udphdr->dest) == 53 && !args->fwd53) return 1;
...
void block_udp(const struct arguments *args, const uint8_t *pkt, size_t length, const uint8_t *payload, int uid) {
    ...
    // 🚨 HARDWARE ALLOCATION FIX: Use raw calloc
...
jboolean handle_udp(const struct arguments *args, const uint8_t *pkt, size_t length, const uint8_t *payload, int uid, struct allowed *redirect, const int epoll_fd) {
    ...
    if (cur != NULL && cur->udp.state != UDP_ACTIVE) return 0;
    ...
    if (ntohs(udphdr->source) == 68 || ntohs(udphdr->dest) == 67) {
        if (check_dhcp(args, &cur->udp, data, datalen) >= 0) return 1;
    }
...
ssize_t write_udp(const struct arguments *args, const struct udp_session *cur, uint8_t *data, size_t datalen) {
    ...
    if (cur->version == 4) {
        ...
        if (datalen) memcpy(buffer + sizeof(struct iphdr) + sizeof(struct udphdr), data, datalen);
        ...
        if (datalen) memcpy(buffer + sizeof(struct ip6_hdr) + sizeof(struct udphdr), data, datalen);
